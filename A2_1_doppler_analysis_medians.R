#########################################################################################
# A2 Script 1: Doppler analysis, using median LI values of all useable trials
#########################################################################################
# This script reads in data from two sessions (1-2) from 6 tasks (A-F) and calcuates 
# Peak and Mean LI values, taking the median value from all trials
...
rawmeansdir<- paste(dir,"Raw Means/", sep = "")
...
for (mysub in c(1:30)){ 
  ...
  for (session in 1:2){
    ...
    trialloc<-paste(dir,"Triallist.xlsx",sep='') # File lists all subjects and trial inclusions/exclusions
    ...
    # Read Triallist.xlsx
    triallist<-read.xlsx(trialloc, sheetIndex = session)
    triallist$Filename<-as.character(triallist$Filename) # unfactor these columns to avoid later difficulties
    
    # Read Results file
    resultsloc <- paste(dir,"Results_MEDIAN_Session",session,".xlsx",sep='') # File lists analysis results
    ...
    comments_col <- (t-1)*9+2
    results[,comments_col]<-as.character(results[,comments_col])
    ...
    results[mysub,(t-1)*9+3]=length(origmarkerlist) #adds number of original markers 
    ...
    #--------------------------------------------------------
    results[mysub,(t-1)*9+4]=length(badpoints) # add number of spiking/dropout points to table for saving
    ...
    results[mysub,(t-1)*9+5]=length(allextreme) #report number of extreme values across trials
    ...
    # For every trial, take L-R peak and calculate L-R mean
    mypeak <- rep("NA", myN)
    mylatency <- rep("NA", myN)
    mymean <- rep("NA", myN)
    for (m in 1:myN){
      # Calculate L-R difference
      Left <- finalepochs[m,,1]
      Right <- finalepochs[m,,2]
      LRdiff=Left - Right
      
      # Take maximum (or minumum) L-R value as peak
      mymax=max(LRdiff[rangestart:rangeend])
      mymin=min(LRdiff[rangestart:rangeend])
      myside=1;
      mylatpeak=mymax
      if(-mymin>mymax) #R peak > L peak
      ...
      mypeak[m]=as.numeric(format(mean(LRdiff[mypeakrange]),digits=4))
      
      # Calculate average L-R value within POI
      mymean[m] <- as.numeric(format(mean(LRdiff[rangestart:rangeend]), digits=4))
      
    }
    
    mypeak <- as.numeric(mypeak)
    mymean <- as.numeric(mymean)
    
    # Take median peak  and mean values
    LI_peak <- median(mypeak)
    LI_mean <- median(mymean)
    ...
    startcol <- (t-1)*9+6
    endcol <- (t-1)*9+10
    results[mysub,startcol:endcol]=c(myN,LI_peak,LI_mean,myse,myse_mean)
